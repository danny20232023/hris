...
const COMPONENT_ID = '201-pds';

const Pds = ({ isOpen, onClose, selectedEmployee, onSave }) => {
  const { can, loading: permissionsLoading } = usePermissions();
  const canReadPds = can(COMPONENT_ID, 'read');
  const canCreatePds = can(COMPONENT_ID, 'create');
  const canUpdatePds = can(COMPONENT_ID, 'update');
  const canPrintPds = can(COMPONENT_ID, 'print');
  const canEditCurrentRecord = selectedEmployee?.hasPDS ? canUpdatePds : canCreatePds;
  // Helper to determine if form inputs should be disabled
  const isFormDisabled = !canEditCurrentRecord || permissionsLoading;
  ...
  // Page 4 - Declarations state
  const [declarations, setDeclarations] = useState({
    third_degree: false,
    fourth_degree: false,
    fourth_degree_details: '',
    guilty_admin: false,
    guilty_admin_details: '',
    thirtyfive_b: false,
    thirtyfive_datefiled: '',
    thirtyfive_statuses: '',
    convicted: false,
    convicted_details: '',
    separated_service: false,
    separated_service_details: '',
    candidate: false,
    candidate_details: '',
    resigned_campaign: false,
  ...
  const resetFormState = () => {
    ...
    setDeclarations({
      ...
      pwd_id_no: '',
      solo_parent_id_no: '',
      authorized_government: false,
      authorized_government_details: ''
    });
  ...
  useEffect(() => {
    if (!isOpen || permissionsLoading || !canReadPds) return;
    fetchLookupData();
  }, [isOpen, permissionsLoading, canReadPds]);

  // Reset form state when modal closes
  useEffect(() => {
    if (!isOpen) {
      console.log('🚪 [PDS] Modal closed, resetting form state');
      resetFormState();
    }
  }, [isOpen]);
  ...
  const retryFetchLookupData = () => {
    if (!canReadPds) {
      alert('You do not have permission to load lookup data.');
      return;
    }
  ...
  // Handle selectedEmployee changes
  useEffect(() => {
    if (!selectedEmployee) return;

    console.log('👤 [PDS] Selected employee changed, resetting form state first');
    resetFormState();

    if (!canReadPds) return;
    
    setTimeout(() => {
      if (selectedEmployee.hasPDS) {
        console.log('📄 [PDS] Loading existing PDS data for employee:', selectedEmployee.dtruserid);
        fetchPDSData(selectedEmployee.dtruserid);
      } else {
        console.log('📝 [PDS] Creating new PDS for employee:', selectedEmployee.dtruserid);
        setFormData(prev => ({
          ...prev,
          dtruserid: selectedEmployee.dtruserid,
          badgenumber: selectedEmployee.badgenumber
        }));
      }
      
      fetchMissingFields(selectedEmployee.dtruserid);
    }, 100);
  }, [selectedEmployee, canReadPds]);

  // Check for missing fields whenever form data or array states change
  useEffect(() => {
    if (!canReadPds || Object.keys(formData).length === 0) return;
    if (selectedEmployee?.dtruserid) {
      fetchMissingFields(selectedEmployee.dtruserid);
    } else {
      checkMissingFields();
    }
  ...
  const fetchPDSData = async (dtruserid) => {
    if (!canReadPds) return;
    try {
      setLoading(true);
      const response = await api.get(`/201-employees/pds/${dtruserid}`);
        ...
        Object.entries(pdsData.employee).map(([key, value]) => {
          ...
          if (key === 'date_of_birth' && value) {
            ...
            if (!isNaN(date.getTime())) {
              ...
              console.log(`📅 [PDS] Converting date: ${value} -> ${formattedDate}`);
      ...
      if (pdsData.eligibility && pdsData.eligibility.length > 0) {
        setCivilServiceEligibility(pdsData.eligibility);
      }
      if (pdsData.workExperience && pdsData.workExperience.length > 0) {
        ...
        // Normalize work experience data to ensure proper boolean values
      ...
      if (pdsData.declarations && Object.keys(pdsData.declarations).length > 0) {
        ...
        const mappedDeclarations = {
          third_degree: convertToBoolean(pdsData.declarations.third_degree),
          fourth_degree: convertToBoolean(pdsData.declarations.fourth_degree),
          fourth_degree_details: pdsData.declarations.fourth_degree_details || '',
          guilty_admin: convertToBoolean(pdsData.declarations.guilty_admin),
          guilty_admin_details: pdsData.declarations.guilty_admin_details || '',
          thirtyfive_b: convertToBoolean(pdsData.declarations.criminally_charged),
          thirtyfive_datefiled: formatDateForInput(pdsData.declarations.criminally_charged_date),
          thirtyfive_statuses: pdsData.declarations.criminally_charged_status || '',
          convicted: convertToBoolean(pdsData.declarations.convicted),
          convicted_details: pdsData.declarations.convicted_details || '',
          separated_service: convertToBoolean(pdsData.declarations.separated_service),
          separated_service_details: pdsData.declarations.separated_service_details || '',
          candidate: convertToBoolean(pdsData.declarations.candidate),
          candidate_details: pdsData.declarations.candidate_details || '',
          resigned_campaign: convertToBoolean(pdsData.declarations.resigned_campaign),
          resigned_campaign_details: pdsData.declarations.resigned_campaign_details || '',
          immigrant: convertToBoolean(pdsData.declarations.immigrant),
          immigrant_country: pdsData.declarations.immigrant_country || '',
          indigenous_group: convertToBoolean(pdsData.declarations.indigenous_group),
          indigenous_group_details: pdsData.declarations.indigenous_group_details || '',
          pwd: convertToBoolean(pdsData.declarations.pwd),
          pwd_id_no: pdsData.declarations.pwd_id_no || '',
          solo_parent: convertToBoolean(pdsData.declarations.solo_parent),
          solo_parent_id_no: pdsData.declarations.solo_parent_id_no || ''
        };
      ...
      if (pdsData.media && Object.keys(pdsData.media).length > 0) {
        ...
        if (pdsData.media.signature && pdsData.media.signature.trim() !== '') {
          console.log('✅ Loading saved signature, length:', pdsData.media.signature.length);
          setSignatureData(pdsData.media.signature);
        } else {
          console.log('❌ No signature data or empty string');
        }
        if (pdsData.media.photo && pdsData.media.photo.trim() !== '') {
          console.log('✅ Loading saved photo, length:', pdsData.media.photo.length);
          setPhotoData(pdsData.media.photo);
        } else {
          console.log('❌ No photo data or empty string');
        }
        if (pdsData.media.thumb && pdsData.media.thumb.trim() !== '') {
          console.log('✅ Loading saved thumbmark, length:', pdsData.media.thumb.length);
          setThumbmarkData(pdsData.media.thumb);
        } else {
          console.log('❌ No thumbmark data or empty string');
        }
        ...
        console.log('❌ No media data received or empty object');
      }
      
      console.log('✅ Loaded existing PDS data for employee:', dtruserid);
    } catch (error) {
  ...
  const fetchLookupData = async () => {
    ...
    try {
      ...
      // Fetch blood types, civil statuses, and eligibility types
      console.log('🔄 [PDS] Starting API calls...');
      const [bloodTypesRes, civilStatusRes, eligibilityTypesRes] = await Promise.all([
        api.get('/201-employees/lookup/blood-types'),
        api.get('/201-employees/lookup/civil-statuses'),
        api.get('/201-employees/eligibility-types')
      ]);
      
      console.log('📊 [PDS] Blood types response:', bloodTypesRes.data);
      console.log('📊 [PDS] Civil statuses response:', civilStatusRes.data);
      console.log('📊 [PDS] Eligibility types response:', eligibilityTypesRes.data);
      
      // Check if responses have the expected structure
      console.log('🔍 [PDS] Blood types data structure:', {
        hasData: !!bloodTypesRes.data.data,
        dataLength: bloodTypesRes.data.data?.length,
        success: bloodTypesRes.data.success
      });
      
      console.log('🔍 [PDS] Civil statuses data structure:', {
        hasData: !!civilStatusRes.data.data,
        dataLength: civilStatusRes.data.data?.length,
        success: civilStatusRes.data.success
      });
      
      console.log('🔍 [PDS] Eligibility types data structure:', {
        hasData: !!eligibilityTypesRes.data.data,
        dataLength: eligibilityTypesRes.data.data?.length,
        success: eligibilityTypesRes.data.success
      });
      
      setBloodTypes(bloodTypesRes.data.data);
      setCivilStatuses(civilStatusRes.data.data);
      setEligibilityTypes(eligibilityTypesRes.data.data);
      console.log('✅ [PDS] Lookup data loaded successfully');
      console.log('📋 [PDS] Eligibility types set:', eligibilityTypesRes.data.data.length, 'items');
    } catch (error) {
  ...
  const handleChange = (e) => {
    if (!canEditCurrentRecord) return;
  ...
  const handleDeclarationToggle = (field, value, detailFields = []) => {
    if (!canEditCurrentRecord) return;
    setDeclarations(prev => {
      const updated = { ...prev, [field]: value };
      if (!value && Array.isArray(detailFields)) {
        detailFields.forEach(detailKey => {
          if (detailKey) {
            updated[detailKey] = '';
          }
        });
  ...
  const fetchMissingFields = async (employeeId) => {
    if (!canReadPds) return;
    try {
      if (!employeeId) return;
      
      console.log('🔍 [PDS] Fetching missing fields for employee:', employeeId);
      const response = await api.get(`/201-employees/missing-fields/${employeeId}`);
  ...
  const handlePrint = () => {
    if (!canPrintPds) {
      alert('You do not have permission to print PDS records.');
      return;
    }
    const currentPage = activePage;
    console.log('🖨️ [PDS Print] Current active page:', currentPage);
    ...
    const targetPrintPage = printPageMapping[currentPage] || 1;
    const printUrl = `/201-employees/print/${selectedEmployee?.dtruserid}?page=${targetPrintPage}`;
  ...
  const handleSubmit = async (isDraft = false) => {
    if (!canEditCurrentRecord) {
      alert('You do not have permission to save this PDS.');
      return;
    }
    ...
    try {
      ...
      const ensureDateFormat = (dateStr) => {
        ...
        console.log('⚠️ Invalid date format for database:', dateStr);
        return null;
      };
      ...
      const mappedDeclarations = {
        third_degree: toTinyInt(declarations.third_degree),
        fourth_degree: toTinyInt(declarations.fourth_degree),
        fourth_degree_details: detailTextValue(declarations.fourth_degree, declarations.fourth_degree_details),
        guilty_admin: toTinyInt(declarations.guilty_admin),
        guilty_admin_details: detailTextValue(declarations.guilty_admin, declarations.guilty_admin_details),
        criminally_charged: toTinyInt(declarations.thirtyfive_b),
        criminally_charged_date: declarations.thirtyfive_b ? ensureDateFormat(declarations.thirtyfive_datefiled) : null,
        criminally_charged_status: detailTextValue(declarations.thirtyfive_b, declarations.thirtyfive_statuses),
        convicted: toTinyInt(declarations.convicted),
        convicted_details: detailTextValue(declarations.convicted, declarations.convicted_details),
        separated_service: toTinyInt(declarations.separated_service),
        separated_service_details: detailTextValue(declarations.separated_service, declarations.separated_service_details),
        candidate: toTinyInt(declarations.candidate),
        candidate_details: detailTextValue(declarations.candidate, declarations.candidate_details),
        resigned_campaign: toTinyInt(declarations.resigned_campaign),
        resigned_campaign_details: detailTextValue(declarations.resigned_campaign, declarations.resigned_campaign_details),
        immigrant: toTinyInt(declarations.immigrant),
        immigrant_country: detailTextValue(declarations.immigrant, declarations.immigrant_country),
        indigenous_group: toTinyInt(declarations.indigenous_group),
        indigenous_group_details: detailTextValue(declarations.indigenous_group, declarations.indigenous_group_details),
        pwd: toTinyInt(declarations.pwd),
        pwd_id_no: detailTextValue(declarations.pwd, declarations.pwd_id_no),
        solo_parent: toTinyInt(declarations.solo_parent),
        solo_parent_id_no: detailTextValue(declarations.solo_parent, declarations.solo_parent_id_no)
      };

      const payload = {
        employee: {
          ...formData,
          objid: selectedEmployee?.objid // Include the selected employee's objid
        },
        children: children.filter(child => child.full_name || child.date_of_birth),
        education: education,
        civil_service_eligibility: civilServiceEligibility.filter(el => el.eligibility_type),
        work_experience: workExperience.filter(work => work.position_title || work.department_agency_company),
        // Page 3 data
        ...
        is_replacing_gov_id: isReplacingGovId,
      ...
      const response = await api.post('/201-employees/pds', payload);
    ...
    <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
          ...
          <div>
            ...
            <select
              ...
              onChange={handleChange}
              className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors.civil_status ? 'border-red-500' : 'border-gray-300'}`}
              required
            >
      ...
      <section id="education-section" className="bg-white rounded-lg shadow p-6">
        ...
        <div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse border border-gray-300 table-fixed">
              <thead className="bg-gray-50">
                <tr>
                  <th className="border border-gray-300 px-4 py-2 text-left w-[10%]">LEVEL</th>
                  <th className="border border-gray-300 px-4 py-2 text-left w-[20%]">NAME OF SCHOOL</th>
                  <th className="border border-gray-300 px-4 py-2 text-left w-[15%]">DEGREE/COURSE</th>
                  <th className="border border-gray-300 px-4 py-2 text-left w-[12%]">PERIOD</th>
                  <th className="border border-gray-300 px-4 py-2 text-left w-[13%]">HIGHEST LEVEL/UNITS</th>
                  <th className="border border-gray-300 px-4 py-2 text-left w-[10%]">YEAR GRADUATED</th>
                  <th className="border border-gray-300 px-4 py-2 text-left w-[20%]">SCHOLARSHIP/HONORS</th>
                </tr>
              </thead>
              <tbody>
          ...
          <p className="text-sm text-gray-500 mt-2">(Continue on separate sheet if necessary)</p>
        </div>
      </section>
    </div>
  );
                  ...
                  <tr key={index}>
                      ...
                      >
                        ...
                        {loadingLookupData ? (
                          ...
                          eligibilityTypes.map(type => (
                            <option key={type.id} value={type.careertypes}>
                              {type.careertypes}
                            </option>
                          ))
                        ) : (
                    ...
                    <td className="border border-gray-300 px-4 py-2">
                      <input 
                        ...
                        type="date" 
                        value={eligibility.date_of_examination} 
                    ...
                    <td className="border border-gray-300 px-4 py-2">
                      <input 
                        ...
                        type="date" 
                        value={eligibility.date_of_validity} 
        ...
        <div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse border border-gray-300">
              <thead className="bg-gray-50">
                <tr>
                  <th className="border border-gray-300 px-4 py-2 text-left">DATE FROM</th>
                  <th className="border border-gray-300 px-4 py-2 text-left">DATE TO / PRESENT</th>
                  <th className="border border-gray-300 px-4 py-2 text-left">POSITION TITLE</th>
                  <th className="border border-gray-300 px-4 py-2 text-left">DEPARTMENT/AGENCY/COMPANY</th>
                  <th className="border border-gray-300 px-4 py-2 text-left">MONTHLY SALARY</th>
                  <th className="border border-gray-300 px-4 py-2 text-left">SALARY/GRADE/STEP</th>
                  <th className="border border-gray-300 px-4 py-2 text-left">STATUS OF APPOINTMENT</th>
                  <th className="border border-gray-300 px-4 py-2 text-left">GOV'T SERVICE</th>
                  <th className="border border-gray-300 px-4 py-2 text-left">ACTION</th>
                </tr>
              </thead>
              <tbody>
                      ...
                      <div className="flex items-center space-x-2">
                        <input 
                          ...
                          value={work.is_present === true ? 
                            (work.date_to || new Date().toISOString().split('T')[0]) : 
                            (work.date_to && work.date_to !== '0' && work.date_to !== '0000-00-00' ? work.date_to : '')
                          } 
        ...
        <div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse border border-gray-300">
              <thead className="bg-gray-50">
                <tr>
                  <th className="border border-gray-300 px-4 py-2 text-left">NAME & ADDRESS OF ORGANIZATION</th>
              ...
              <div className="flex space-x-6 mb-2">
                <label className="flex items-center">
                  <input
                    ...
                    name="third_degree"
                    value="true"
                    checked={declarations.third_degree === true}
                    onChange={() => handleDeclarationToggle('third_degree', true)}
                    className="mr-2"
                  />
                  YES
                </label>
                <label className="flex items-center">
                  <input
                    ...
                    name="third_degree"
                    value="false"
                    checked={declarations.third_degree === false}
                    onChange={() => handleDeclarationToggle('third_degree', false)}
                    className="mr-2"
                  />
                  NO
                </label>
          ...
          <div className="grid grid-cols-[2fr,1fr] gap-4 items-start">
            ...
            <div>
              <div className="flex space-x-6 mb-2">
                <label className="flex items-center">
                  <input
                    ...
                    name="fourth_degree"
                    value="true"
                    checked={declarations.fourth_degree === true}
                    onChange={() => handleDeclarationToggle('fourth_degree', true, ['fourth_degree_details'])}
                    className="mr-2"
                  />
                  YES
                </label>
                <label className="flex items-center">
                  <input
                    ...
                    name="fourth_degree"
                    value="false"
                    checked={declarations.fourth_degree === false}
                    onChange={() => handleDeclarationToggle('fourth_degree', false, ['fourth_degree_details'])}
                    className="mr-2"
                  />
                  NO
                </label>
              ...
              {declarations.fourth_degree && (
                <input
                  type="text"
                  name="fourth_degree_details"
                  value={declarations.fourth_degree_details}
              ...
              <div className="flex space-x-6 mb-2">
                <label className="flex items-center">
                  <input
                    ...
                    name="guilty_admin"
                    value="true"
                    checked={declarations.guilty_admin === true}
                    onChange={() => handleDeclarationToggle('guilty_admin', true, ['guilty_admin_details'])}
                    className="mr-2"
                  />
                  YES
                </label>
                <label className="flex items-center">
                  <input
                    ...
                    name="guilty_admin"
                    value="false"
                    checked={declarations.guilty_admin === false}
                    onChange={() => handleDeclarationToggle('guilty_admin', false, ['guilty_admin_details'])}
                    className="mr-2"
                  />
                  NO
                </label>
              </div>
              {declarations.guilty_admin && (
                <input
                  type="text"
                  name="guilty_admin_details"
                  value={declarations.guilty_admin_details}
                  onChange={handleDeclarationChange}
                  placeholder="If YES, give details:"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm mt-2"
                />
            ...
            <div>
              <div className="flex space-x-6 mb-2">
                <label className="flex items-center">
                  <input
                    ...
                    name="convicted"
                    value="true"
                    checked={declarations.convicted === true}
                    onChange={() => handleDeclarationToggle('convicted', true, ['convicted_details'])}
                    className="mr-2"
                  />
                  YES
                </label>
                <label className="flex items-center">
                  <input
                    ...
                    name="convicted"
                    value="false"
                    checked={declarations.convicted === false}
                    onChange={() => handleDeclarationToggle('convicted', false, ['convicted_details'])}
                    className="mr-2"
                  />
                  NO
                </label>
              ...
              {declarations.convicted && (
                <input
                  ...
                  value={declarations.convicted_details}
                  onChange={handleDeclarationChange}
                  placeholder="If YES, give details:"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm mt-2"
                />
            ...
            <div>
              <div className="flex space-x-6 mb-2">
                <label className="flex items-center">
                  <input
                    type="radio"
                    name="separated_service"
                    value="true"
                    checked={declarations.separated_service === true}
                    onChange={() => handleDeclarationToggle('separated_service', true, ['separated_service_details'])}
                    className="mr-2"
                  />
                  YES
                </label>
                <label className="flex items-center">
                  <input
                    ...
                    name="separated_service"
                    value="false"
                    checked={declarations.separated_service === false}
                    onChange={() => handleDeclarationToggle('separated_service', false, ['separated_service_details'])}
                    className="mr-2"
                  />
                  NO
                </label>
              ...
              {declarations.separated_service && (
                <input
                  type="text"
                  name="separated_service_details"
                  value={declarations.separated_service_details}
                  onChange={handleDeclarationChange}
                  placeholder="If YES, give details:"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm mt-2"
                />
            ...
            <div>
              <div className="flex space-x-6 mb-2">
                <label className="flex items-center">
                  <input
                    ...
                    name="candidate"
                    value="true"
                    checked={declarations.candidate === true}
                    onChange={() => handleDeclarationToggle('candidate', true, ['candidate_details'])}
                    className="mr-2"
                  />
                  YES
                </label>
                <label className="flex items-center">
                  <input
                    ...
                    name="candidate"
                    value="false"
                    checked={declarations.candidate === false}
                    onChange={() => handleDeclarationToggle('candidate', false, ['candidate_details'])}
                    className="mr-2"
                  />
                  NO
                </label>
              ...
              {declarations.candidate && (
                <input
                  ...
                  value={declarations.candidate_details}
                  onChange={handleDeclarationChange}
                  placeholder="If YES, give details:"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm mt-2"
                />
            ...
            <div>
              <div className="flex space-x-6 mb-2">
                <label className="flex items-center">
                  <input
                    type="radio"
                    name="resigned_campaign"
                    value="true"
                    checked={declarations.resigned_campaign === true}
                    onChange={() => handleDeclarationToggle('resigned_campaign', true, ['resigned_campaign_details'])}
                    className="mr-2"
                  />
                  YES
                </label>
                <label className="flex items-center">
                  <input
                    ...
                    name="resigned_campaign"
                    value="false"
                    checked={declarations.resigned_campaign === false}
                    onChange={() => handleDeclarationToggle('resigned_campaign', false, ['resigned_campaign_details'])}
                    className="mr-2"
                  />
                  NO
                </label>
              </div>
              {declarations.resigned_campaign && (
                <input
                  type="text"
                  name="resigned_campaign_details"
                  value={declarations.resigned_campaign_details}
                  onChange={handleDeclarationChange}
                  placeholder="If YES, give details:"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm mt-2"
                />
              ...
              <div className="flex space-x-6 mb-2">
                <label className="flex items-center">
                  <input
                    ...
                    name="immigrant"
                    value="true"
                    checked={declarations.immigrant === true}
                    onChange={() => handleDeclarationToggle('immigrant', true, ['immigrant_country'])}
                    className="mr-2"
                  />
                  YES
                </label>
                <label className="flex items-center">
                  <input
                    ...
                    name="immigrant"
                    value="false"
                    checked={declarations.immigrant === false}
                    onChange={() => handleDeclarationToggle('immigrant', false, ['immigrant_country'])}
                    className="mr-2"
                  />
                  NO
                </label>
              </div>
              {declarations.immigrant && (
                <input
                  type="text"
                  name="immigrant_country"
                  value={declarations.immigrant_country}
                  onChange={handleDeclarationChange}
          ...
          <div className="grid grid-cols-[2fr,1fr] gap-4 items-start">
            ...
            <div>
              <div className="flex space-x-6 mb-2">
                <label className="flex items-center">
                  <input
                    ...
                    value="true"
                    checked={declarations.indigenous_group === true}
                    onChange={() => handleDeclarationToggle('indigenous_group', true, ['indigenous_group_details'])}
                    className="mr-2"
                  />
                  YES
                </label>
                <label className="flex items-center">
                  <input
                    ...
                    name="indigenous_group"
                    value="false"
                    checked={declarations.indigenous_group === false}
                    onChange={() => handleDeclarationToggle('indigenous_group', false, ['indigenous_group_details'])}
                    className="mr-2"
                  />
                  NO
                </label>
              ...
              {declarations.indigenous_group && (
                <input
                  type="text"
                  name="indigenous_group_details"
                  value={declarations.indigenous_group_details}
                  onChange={handleDeclarationChange}
              ...
              <div className="flex space-x-6 mb-2">
                <label className="flex items-center">
                  <input
                    ...
                    name="pwd"
                    value="true"
                    checked={declarations.pwd === true}
                    onChange={() => handleDeclarationToggle('pwd', true, ['pwd_id_no'])}
                    className="mr-2"
                  />
                  YES
                </label>
                <label className="flex items-center">
                  <input
                    ...
                    name="pwd"
                    value="false"
                    checked={declarations.pwd === false}
                    onChange={() => handleDeclarationToggle('pwd', false, ['pwd_id_no'])}
                    className="mr-2"
                  />
                  NO
                </label>
              ...
              <div className="flex space-x-6 mb-2">
                <label className="flex items-center">
                  <input
                    ...
                    name="solo_parent"
                    value="true"
                    checked={declarations.solo_parent === true}
                    onChange={() => handleDeclarationToggle('solo_parent', true, ['solo_parent_id_no'])}
                    className="mr-2"
                  />
                  YES
                </label>
                <label className="flex items-center">
                  <input
                    ...
                    name="solo_parent"
                    value="false"
                    checked={declarations.solo_parent === false}
                    onChange={() => handleDeclarationToggle('solo_parent', false, ['solo_parent_id_no'])}
                    className="mr-2"
                  />
                  NO
                </label>
              </div>
              {declarations.solo_parent && (
                <input
                  type="text"
                  name="solo_parent_id_no"
                  value={declarations.solo_parent_id_no}
                  onChange={handleDeclarationChange}
                  placeholder="If YES, please specify ID No:"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm mt-2"
                />
        ...
        <div>
          ...
          <div className="space-y-4">
            {references.map((ref, index) => {
              const isComplete = ref.reference_name && ref.reference_name.trim() !== '';
              return (
              <div key={index} className={`grid grid-cols-1 md:grid-cols-4 gap-4 p-4 border rounded-lg ${
                isComplete ? 'border-green-300 bg-green-50' : 'border-gray-200 bg-white'
              }`}>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">41.1 Name</label>
                  <input
                    type="text"
                ...
                <div className="md:col-span-2">
                  <label className="block text-sm font-medium text-gray-700 mb-1">41.2 Address</label>
                  <input
                    type="text"
                ...
                <div className="flex space-x-2">
                  <div className="flex-1">
                    <label className="block text-sm font-medium text-gray-700 mb-1">41.3 Tel. No.</label>
                    <input
                      type="text"
          ...
          <div className="space-y-4">
            {governmentIds.map((id, index) => (
              <div key={index} className={`grid grid-cols-1 md:grid-cols-5 gap-4 p-4 border rounded-lg ${
                isReplacingGovId ? 'border-orange-300 bg-orange-50' : 'border-gray-200 bg-white'
              }`}>
                <input
                  type="text"
                  value={id.government_issued_id}
                  onChange={(e) => updateGovernmentId(index, 'government_issued_id', e.target.value)}
                  placeholder="43.1 Government Issued ID"
                ...
                <input
                  ...
                  placeholder="43.4 Place of Issuance"
                  className={`px-3 py-2 border rounded-lg ${
                    isReplacingGovId ? 'border-orange-300 focus:border-orange-500' : 'border-gray-300'
                  }`}
                  required={isReplacingGovId}
          ...
          <div className="mt-4 flex space-x-3">
            {!isReplacingGovId ? (
              ...
              <>
                <button 
                  type="button" 
                  onClick={acceptGovernmentIdChange} 
                  className="px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600"
                >
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 overflow-y-auto">
      <div className="bg-white rounded-lg shadow-xl max-w-7xl w-full mx-4 my-8 max-h-[90vh] overflow-y-auto">
        {/* Modal Header */}
        <div className="sticky top-0 bg-gradient-to-r from-blue-600 to-indigo-700 rounded-t-lg shadow-lg px-6 py-4 flex justify-between items-center z-10">
          <div>
            <h2 className="text-2xl font-bold text-white mb-1">
              {selectedEmployee?.hasPDS ? '📝 Edit' : '➕ Add'} Personal Data Sheet (PDS)
            </h2>
            <p className="text-blue-100 text-sm">
              CS Form No. 212 (Revised 2017) - {selectedEmployee?.name || 'Employee'}
            </p>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex space-x-3">
            <button
              ...
              disabled={loading || !canEditCurrentRecord || permissionsLoading}
              className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-2"
              title={!canEditCurrentRecord ? 'You do not have permission to save PDS records' : ''}
            >
              {loading ? (
                <>
            ...
            <button
              onClick={handlePrint}
              disabled={!canPrintPds || permissionsLoading}
              className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-2"
              title={!canPrintPds ? 'You do not have permission to print PDS records' : ''}
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
